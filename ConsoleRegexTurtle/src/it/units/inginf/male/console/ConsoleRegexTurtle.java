    ...
    static private void parseArgs(String[] args, SimpleConfig simpleConfig){
        try{
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                switch (string){
                    case "-t": simpleConfig.numberThreads = Integer.valueOf(parameter);break;
                    case "-p": simpleConfig.populationSize = Integer.valueOf(parameter);break;
                    case "-d": simpleConfig.datasetName = parameter;
                               mandatoryDatasetCheck = false;
                               break;
                    case "-o": simpleConfig.outputFolder = parameter;break;
                    case "-g": simpleConfig.generations = Integer.valueOf(parameter);break;
                    case "-j": simpleConfig.numberOfJobs = Integer.valueOf(parameter);break;
                    case "-e": simpleConfig.termination = Double.valueOf(parameter);break;
                    case "-x": simpleConfig.populateOptionalFields = Boolean.valueOf(parameter);break;
                    case "-h": System.out.println(HELP_MESSAGE);break;
                    case "-c": simpleConfig.comment = parameter;break;
                    case "-s": simpleConfig.isStriped = Boolean.valueOf(parameter);break;
                }    
            }
